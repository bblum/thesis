...
\begin{figure*}[t!]
	...
	All numbers represent the average from 5 trials, with the standard deviation shown in parentheses. ``\nobugnum{$X$}{$s$}'' indicates the bug was not found with a given test setup (and instead, Landslide took $X$ time to explore the entire decision tree). ``$\infty$'' indicates the bug was not found with stress testing (after 1 hour), or that Landslide's search did not finish (after 8 hours). % TODO: after how long? what timeout?
	}
	\label{fig:numbers}
\end{figure*}
