...
\begin{quote}
\bf
Systematic exploration in kernel space is made possible by instrumenting the kernel's internal abstractions to understand its concurrency, and made efficient by relying on the user to control the scope of the test.
\end{quote}
}

Landslide\footnote{
Landslide {\em (n)} - A phenomenon which demonstrates that Pebbles are not as stable as you might think.}
is an effort to make possible the use of systematic exploration in kernel-space.
