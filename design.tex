...
Landslide also maintains a set of memory accesses. % TODO
...
\begin{algorithm*}[t!]
	...
	\begin{algorithmic}
		...
		\\
		\Function{scheduler\_update}{int eip}
			\If{schedule\_in\_flight}
				\State {\sc assert}(in\_timer\_handler || in\_context\_switch);
				\If { {\sc kernel\_exiting\_timer}(eip) || (!handling\_timer \&\& {\sc kernel\_exiting\_context\_switch}(eip))}
					\State {\em Then the kernel has just finished rescheduling and is about to resume normal thread execution.}
		...
		\EndFunction
	\end{algorithmic}
	\caption{The scheduling algorithm. This procedure for updating the scheduler's state is executed once per instruction, with a corresponding value for \texttt{eip} each time. The predicates on \texttt{eip} are part of the kernel instrumentation which tell what the guest kernel is doing at the current instruction.}
\end{algorithm*}
...
\begin{enumerate}
	\item Kernel panic bugs - If the kernel invoked \texttt{panic}, it detected its own bug, and Landslide need do nothing but report it.
	\item Use-after-free bugs - Whenever the kernel accesses memory in the heap (when not in the dynamic allocator itself), Landslide verifies that the address is within an allocated range. If not, Landslide proclaims the access to be illegal.
	\item Deadlock bugs -
...
% talk about ignoring runqueue accesses, etc TODO
