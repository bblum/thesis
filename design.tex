...
\subsection{Landslide Scheduling}
...
In Landslide's current model, timer interrupts are the only source of nondeterminism for the guest kernel, and controlling when they occur theoretically allows for complete control over the concurrent behaviour of the test case. (We discuss expanding this limited model in Section~\ref{sec:future-drivers}.) However, it is more useful to view the concurrent behaviour with a higher-level abstraction, in terms of the set of runnable threads and the ability to preempt the currently-running thread with any different runnable one.
...
\cite{dpor} and \cite{dbug-retreat} present a partial order reduction algorithm in which only the first ``evil ancestor'' (term defined in \cite{dbug-retreat}) of each transition need be considered when identifying which alternate interleavings need to be explored.
...
\begin{figure}[h]
	...
	\subfloat[Race-free interleaving.]{
		...
		\begin{tabular}{l|c|c|c|}
			...
			$C$ & & & \texttt{x=42;} \\
			$B$ & & \texttt{y=31337;} & \\
			\multirow{2}{*}{$A_1$} & \texttt{if (y==31337)} & & \\
			& \texttt{assert(x==42);} & & \\
			\cline{2-4}
		\end{tabular}
	} &
	...
	\subfloat[Buggy interleaving.]{
		...
		\begin{tabular}{l|c|c|c|}
			...
			\cline{2-4}
			$B$ & & \texttt{y=31337;} & \\
			\multirow{2}{*}{$A_3$} & \texttt{if (y==31337)} & & \\
			& \texttt{assert(x==42);} & & \\
			$C$ & & & \texttt{x=42;} \\
			\cline{2-4}
		\end{tabular}
	...
	\end{tabular}
	\caption{This sample code defeats a DPOR implementation that only uses the first evil ancestor if a decision point is not defined between the two lines of thread 1's code. The first interleaving is explored first; DPOR identifies transition $B$ as the first evil ancestor of $A_1$, and runs the second interleaving next. In the second interleaving, $C$ is no longer a second evil ancestor of $A_2$, so the third interleaving is never explored, and the assertion failure is missed.}
	\label{fig:evil-ancestors}
\end{figure}

We found that implementing DPOR in this way was not sound in our setup, because it relies on the invariant that each transition contains only one inter-thread communication event (in kernel-space, this means a shared memory access). Because Landslide builds its decision trees with coarser granularity, i.e. multiple different shared memory conflicts may happen in each transition, Landslide must consider every evil ancestor of each transition when performing DPOR.

Figure~\ref{fig:evil-ancestors} demonstrates a counterexample to the soundness of only identifying one evil ancestor per transition given that transitions are allowed to have multiple different memory conflicts.

While DPOR is a proved-sound algorithm in environments that do have this memory access property, even with this every-ancestor change we are not certain if it is sound in our environment. We justify the use of DPOR anyway because Landslide is already prone to false negatives because of coarse interleaving granularity, and leave proving soundness to future work (and to the theoreticians).
