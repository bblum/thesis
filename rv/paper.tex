...
% remove dbug-retreat citation if need be
...
% Among other systematic testing tools are CHESS \cite{chess}, which heuristically orders its search by number of preemptions; MaceMC \cite{macemc} and MODIST \cite{modist}, which provide systematic testing frameworks for networked and distributed applications; RacePRO \cite{racepro}, which uncovers races between system calls made by multiple processes; and DeMeter \cite{demeter}, which focuses its search on only one component of a system at a time.
...
% FIXME: also say 'on voluntary rescheds'?
