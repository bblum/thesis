...
Landslide's model is also restricted to uniprocessor execution. Supporting multiprocessor environments, device driver testing, and production kernels such as Linux is left to future work.
...
The design of Landslide's exploration is based around {\em decision points}. These are places during the execution at which we force preemptions and around which we cause different threads to interleave. Searching with few decision points results in coarser-grained interleavings, smaller state spaces, and less likelihood of finding bugs; searching with more results in the opposite.
...
In general, with false-negative bug detection, the kernel might execute a buggy behaviour yet Landslide would miss it. However, when Landslide does identify a bug, the user can be sure that a race exists.
...
{\bf Configuring decision points.} While Landslide automatically identifies decision points on voluntary reschedules, these will result in coarse-grained interleavings likely to overlook bugs. After successfully instrumenting their kernel and exploring a minimal state space, the user should then add more decision points for a finer-grained search. We provide an extra annotation, \texttt{tell\_landslide\_decide()}, for this purpose. We recommend using it in concurrency primitives, such as at the start of \texttt{mutex\_lock()} and at the end of \texttt{mutex\_unlock()}.

This strategy may cause Landslide to identify decision points in unrelated parts of the kernel, such as when accessing mutexes in unrelated system calls. We provide an interface for the user to limit decision points to within relevant functions: the user may write ``\texttt{within\_function foo}'' to whitelist function \texttt{foo}, and ``\texttt{without\_function bar}'' to blacklist function \texttt{bar}. For example, if testing thread death and reaping, the user should write \texttt{within\_function exit} and \texttt{within\_function wait}.
...
Certain other shared memory conflicts also arise that are irrelevant to whichever system calls are being tested. If testing for races in thread livecycle routines, the user likely does not care about accesses to the frame allocator mutex.
